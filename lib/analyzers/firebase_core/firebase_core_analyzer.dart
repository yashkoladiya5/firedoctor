...
final class FirebaseCoreAnalyzer extends Analyzer {
  ...
  @override
  String get description => 'Analyzes Firebase Core initialization in Flutter projects';
  ...
  Future<DiagnosticResult> analyze(AnalyzerContext context) async {
    ...
    for (final filePath in dartFiles) {
      ...
      for (int i = 0; i < lines.length; i++) {
        ...
        if (line.contains('Firebase.initializeApp(')) {
          ...
          final isAwaited = line.contains('await') && line.indexOf('await') < initIndex;
    ...
    final firebaseOptionsPath = fs.join(projectPath, 'lib', 'firebase_options.dart');
    ...
    if (initCalls.isEmpty && hasFirebaseCore) {
      issues.add(DiagnosticIssue(
        ...
        description: 'No Firebase.initializeApp() call found in any Dart file under lib/.',
    ...
    if (!hasFirebaseOptionsFile && hasFirebaseCore) {
      issues.add(const DiagnosticIssue(
        ...
        recommendation: 'Run "flutterfire configure" to generate firebase_options.dart.',
      ));
    }
    ...
    if (initCalls.isNotEmpty) {
      ...
      if (shouldEmit) {
        issues.add(const DiagnosticIssue(
          ...
          description: 'WidgetsFlutterBinding.ensureInitialized() should be called '
              'before Firebase.initializeApp() to ensure plugin initialization.',
          recommendation: 'Add WidgetsFlutterBinding.ensureInitialized() before '
    ...
    if (initCalls.length > 1) {
      issues.add(DiagnosticIssue(
        ...
        description: 'Found ${initCalls.length} calls to Firebase.initializeApp(). '
    ...
    if (!hasFirebaseCore && initCalls.isNotEmpty) {
      issues.add(DiagnosticIssue(
        ...
        description: 'firebase_core is not listed in pubspec.yaml dependencies. '
