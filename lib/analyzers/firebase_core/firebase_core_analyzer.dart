...
final class FirebaseCoreAnalyzer extends Analyzer {
  ...
  Future<DiagnosticResult> analyze(AnalyzerContext context) async {
    ...
    final hasFirebaseCore =
        pubspec.hasDependency('firebase_core') ||
        pubspec.hasDevDependency('firebase_core');

    final firebaseOptionsPath = fs.join(
      projectPath,
      'lib',
    ...
    if (initCalls.isEmpty && hasFirebaseCore) {
      issues.add(
        DiagnosticIssue(
          ...
          recommendation:
              'Add Firebase.initializeApp() to your main.dart:\n'
    ...
    if (!hasFirebaseOptionsFile && hasFirebaseCore) {
      issues.add(
        const DiagnosticIssue(
          ...
          description:
              'The lib/firebase_options.dart file does not exist. '
    ...
    if (!foundDefaultOptions && hasFirebaseCore) {
      issues.add(
        const DiagnosticIssue(
          ...
          description:
              'DefaultFirebaseOptions.currentPlatform is not referenced '
              'in any Dart file. Firebase initialization should use '
              'platform-specific options.',
          recommendation:
              'Pass DefaultFirebaseOptions.currentPlatform to '
              'Firebase.initializeApp():\n'
              '  await Firebase.initializeApp(\n'
              '    options: DefaultFirebaseOptions.currentPlatform,\n'
              '  );',
    ...
    if (initCalls.length > 1) {
      issues.add(
        DiagnosticIssue(
          ...
          recommendation:
              'Ensure Firebase.initializeApp() is called only once '
              'in your application.',
        ),
      );
    }
      ...
      if (!call.isAwaited) {
        issues.add(
          DiagnosticIssue(
            ...
            description:
                'Firebase.initializeApp() is called without await on '
                'line ${call.lineNumber}.',
            recommendation:
                'Add await before Firebase.initializeApp():\n'
                '  await Firebase.initializeApp();',
      ...
      if (rAppLine != null && call.lineNumber > rAppLine) {
        issues.add(
          DiagnosticIssue(
            ...
            description:
                'Firebase.initializeApp() is called after runApp() in '
                '${call.filePath}. Firebase should be initialized before runApp().',
            recommendation:
                'Move Firebase.initializeApp() before runApp() '
  ...
  List<_InitCall> _findInitCalls(String content, String filePath) {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      while (true) {
        ...
        final afterCall = line.substring(
          idx + 'Firebase.initializeApp('.length,
        );
