  ...
  Future<DiagnosticResult> analyze(AnalyzerContext context) async {
    ...
    if (!fs.exists(googleServicePath)) {
      issues.add(DiagnosticIssue(
        ...
        description: 'The ios/Runner/GoogleService-Info.plist file is missing. '
    ...
    if (!fs.exists(podfilePath)) {
      issues.add(DiagnosticIssue(
        ...
        recommendation: 'Run "flutter create ." in the ios directory or '
    ...
    ({
      List<String> backgroundModes,
      bool hasFirebaseAppDelegateProxy,
    })? infoPlistInfo;
      ...
      for (final entry in entries) {
        if (fs.isDirectory(entry)) {
          ...
          try {
            ...
            if (RegExp(r'''import\s+['"]package:firebase_''').hasMatch(content)) {
              return true;
            }
          } catch (_) {
          }
