...
FakeFileSystem _createProjectWithMain(String mainContent, {
...
''';
      ...
      test('returns passed with no FD300/FD302/FD306 when everything is correct',
          () async {
      ...
      test('does not produce FD300 when Firebase.initializeApp is present',
          () async {
        final fs = _createProjectWithMain(mainContent, addFirebaseOptions: true);
        final context = AnalyzerContext(projectPath: '/project', fileSystem: fs);
        final result = await analyzer.analyze(context);
      ...
      test('does not produce FD302 when ensureInitialized is present and before init',
          () async {
        final fs = _createProjectWithMain(mainContent, addFirebaseOptions: true);
        final context = AnalyzerContext(projectPath: '/project', fileSystem: fs);
        final result = await analyzer.analyze(context);
    ...
    group('missing initialization (FD300)', () {
      test('returns critical FD300 when no Firebase.initializeApp found in any file',
          () async {
        const noInitContent = '''
import 'package:flutter/material.dart';
...
''';
    ...
    group('missing firebase_options.dart (FD301)', () {
      test('returns warning FD301 when lib/firebase_options.dart does not exist',
          () async {
        const content = '''
import 'package:firebase_core/firebase_core.dart';
...
''';
    ...
    group('missing ensureInitialized (FD302)', () {
      test('returns error FD302 when WidgetsFlutterBinding.ensureInitialized is missing',
          () async {
        const content = '''
import 'package:firebase_core/firebase_core.dart';
...
''';
      ...
      test('returns error FD302 when ensureInitialized appears after Firebase.initializeApp in same file',
          () async {
        const content = '''
import 'package:firebase_core/firebase_core.dart';
...
''';
    ...
    group('DefaultFirebaseOptions not used (FD303)', () {
      test('returns info FD303 when DefaultFirebaseOptions.currentPlatform is not referenced',
          () async {
        const content = '''
import 'package:firebase_core/firebase_core.dart';
...
''';
    ...
    group('multiple init calls (FD304)', () {
      test('returns warning FD304 when Firebase.initializeApp appears multiple times in one file',
          () async {
        const content = '''
import 'package:firebase_core/firebase_core.dart';
...
''';
      ...
      test('returns warning FD304 when Firebase.initializeApp appears in multiple files',
          () async {
        final fs = _createProjectWithMain(
          '''import 'package:firebase_core/firebase_core.dart';
void main() async {
...
''',
    ...
    group('missing await (FD305)', () {
      test('returns warning FD305 when Firebase.initializeApp is called without await on the same line',
          () async {
        const content = '''
import 'package:firebase_core/firebase_core.dart';
...
''';
    ...
    group('init after runApp (FD306)', () {
      test('returns error FD306 when Firebase.initializeApp appears after runApp in the same file',
          () async {
        const content = '''
import 'package:firebase_core/firebase_core.dart';
...
''';
    ...
    group('missing firebase_core dependency (FD307)', () {
      test('returns error FD307 when firebase_core is not in dependencies', () async {
        const content = '''
import 'package:firebase_core/firebase_core.dart';
...
''';
      ...
      test('does not produce FD307 when firebase_core is in dependencies', () async {
        const content = '''
import 'package:firebase_core/firebase_core.dart';
...
''';
    ...
    group('edge cases', () {
      test('handles empty lib/ directory with firebase_core dep — produces FD300 + FD301', () async {
        final fs = _createProject(
          pubspecContent: 'name: test_app\ndependencies:\n  firebase_core: ^3.0.0\ndev_dependencies: {}\n',
        );
        final context = AnalyzerContext(projectPath: '/project', fileSystem: fs);
      ...
      test('handles empty lib/ directory without firebase_core dep — produces no issues', () async {
        final fs = _createProject(
          pubspecContent: 'name: test_app\ndependencies: {}\ndev_dependencies: {}\n',
        );
        final context = AnalyzerContext(projectPath: '/project', fileSystem: fs);
      ...
      test('handles .dart files with no Firebase references — no FD issues triggered', () async {
        const content = '''
import 'package:flutter/material.dart';
...
''';
      ...
      test('handles multiline strings and comments that contain Firebase patterns',
          () async {
        const content = '''
import 'package:firebase_core/firebase_core.dart';
