...
String plistWithBundleId(String bundleId) => '''<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
    <key>BUNDLE_ID</key>
...
String infoPlistWithBackgroundModes(String mode) => '''<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
...
</plist>'''.replaceAll(r'$mode', mode);

String get infoPlistWithFirebaseProxyEnabled => '''<?xml version="1.0" encoding="UTF-8"?>
<plist version="1.0">
<dict>
\t<key>UIBackgroundModes</key>
\t<array>
...
String get infoPlistNoBackgroundModes => '''<?xml version="1.0" encoding="UTF-8"?>
<plist version="1.0">
<dict>
\t<key>FirebaseAppDelegateProxyEnabled</key>
\t<false/>
  ...
  group('IOSAnalyzer', () {
    ...
    group('metadata', () {
      ...
      test('has correct description', () {
        expect(analyzer.description, equals('Analyzes iOS Firebase configuration'));
      });
    ...
    group('GoogleService-Info.plist checks', () {
      ...
      test('emits FD501 when GoogleService-Info.plist has invalid content',
          () async {
        final fs = _createProject(files: {
      ...
      test('emits FD501 for empty plist content', () async {
        final fs = _createProject(files: {
          '/project/ios/Runner/GoogleService-Info.plist': emptyPlist,
        });
        final context =
            AnalyzerContext(projectPath: '/project', fileSystem: fs);
        final result = await analyzer.analyze(context);
    ...
    group('Bundle ID checks', () {
      test('emits FD502 when bundle ID mismatches between plist and pbxproj',
          () async {
        final fs = _createProject(files: {
          '/project/ios/Runner/GoogleService-Info.plist':
              plistWithBundleId('com.example.plistapp'),
      ...
      test('does not emit FD502 when bundle IDs match', () async {
        final fs = _createProject(files: {
          '/project/ios/Runner/GoogleService-Info.plist':
              plistWithBundleId('com.example.testapp'),
          '/project/ios/Runner.xcodeproj/project.pbxproj':
      ...
      test('does not emit FD502 when plist has no BUNDLE_ID', () async {
        final fs = _createProject(files: {
          '/project/ios/Runner/GoogleService-Info.plist': minimalPlistNoBundleId,
      ...
      test('emits FD503 when bundle identifier not detected in pbxproj',
          () async {
        final fs = _createProject(files: {
          '/project/ios/Runner/GoogleService-Info.plist': validPlist,
          '/project/ios/Runner.xcodeproj/project.pbxproj': pbxprojNoBundleId,
          '/project/lib/main.dart': dartFileWithoutFirebase,
        });
        final context =
            AnalyzerContext(projectPath: '/project', fileSystem: fs);
    ...
    group('Runner target checks', () {
      test('emits FD504 when Runner target not found in Xcode project',
          () async {
        final fs = _createProject(files: {
          '/project/ios/Runner/GoogleService-Info.plist': validPlist,
          '/project/ios/Runner.xcodeproj/project.pbxproj':
    ...
    group('Push Notifications capability checks', () {
      test('emits FD505 when Push Notifications capability is missing',
          () async {
        final fs = _createProject(files: {
          '/project/ios/Runner/GoogleService-Info.plist': validPlist,
    ...
    group('Background Modes capability checks', () {
      test('emits FD506 when Background Modes capability is missing',
          () async {
        final fs = _createProject(files: {
          '/project/ios/Runner/GoogleService-Info.plist': validPlist,
          '/project/ios/Runner.xcodeproj/project.pbxproj':
    ...
    group('Remote Notifications background mode checks', () {
      test('emits FD507 when remote-notification background mode is missing',
          () async {
        final fs = _createProject(files: {
          '/project/ios/Runner/GoogleService-Info.plist': validPlist,
      ...
      test('does not emit FD507 when remote-notification is configured',
          () async {
        final fs = _createProject(files: {
          '/project/ios/Runner/GoogleService-Info.plist': validPlist,
    ...
    group('Firebase pods checks', () {
      ...
      test('detects Firebase pods from Podfile.lock when Podfile has none',
          () async {
        final fs = _createProject(files: {
          '/project/ios/Runner/GoogleService-Info.plist': validPlist,
          '/project/ios/Podfile': podfileWithoutFirebase,
    ...
    group('Firebase configuration reference checks', () {
      test('emits FD511 when GoogleService-Info.plist exists but no Firebase imports',
          () async {
        final fs = _createProject(files: {
          '/project/ios/Runner/GoogleService-Info.plist': validPlist,
          '/project/lib/main.dart': dartFileWithoutFirebase,
    ...
    group('APNs configuration checks', () {
      test('emits FD512 when FirebaseAppDelegateProxyEnabled is not configured',
          () async {
        final fs = _createProject(files: {
          '/project/ios/Runner/GoogleService-Info.plist': validPlist,
      ...
      test('does not emit FD512 when FirebaseAppDelegateProxyEnabled is set',
          () async {
        final fs = _createProject(files: {
          '/project/ios/Runner/GoogleService-Info.plist': validPlist,
          '/project/ios/Runner/Info.plist': infoPlistWithFirebaseProxyEnabled,
    ...
    group('happy path - valid iOS Firebase project', () {
      ...
      test('returns passed with Podfile.lock instead of Podfile Firebase pods',
          () async {
        final fs = _createProject(files: {
          '/project/ios/Runner/GoogleService-Info.plist': validPlist,
          '/project/ios/Runner.xcodeproj/project.pbxproj': validPbxproj,
